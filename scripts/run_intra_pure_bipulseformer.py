"""
Cross-dataset 학습-평가 — BiPulseFormer (PhysFormer + BiLevel Routing, ANN).
...
EXPERIMENTS = [
    ('PURE', PURE_PATH, 'PURE', PURE_PATH),
]

RESULT_DIR = 'results/intra_pure_bipulseformer'
LOG = os.path.join(RESULT_DIR, 'log.txt')
LIVE_FILE = os.path.join('results', 'live_progress_intra_pure.md')
STATUS_FILE = os.path.join('results', 'current_status_intra_pure.json')
...
def _live(now, status):
    lines = [
        '# 📊 BiPulseFormer (ANN, BiFormer) — Cross-Dataset',
        '',
...
def main():
    ...
    log(f"[*] BiPulseFormer (PhysFormer official + BiLevel Routing Attention)")
    ...
    try:
        ...
        log("\n" + "=" * 70)
        log("[*] 종합 결과 — BiPulseFormer (ANN, BiFormer)")
        log("=" * 70)
